...
shinyServer(function(input, output, session) {
  ...
  datainfo <- eventReactive(input$file, {
    ...
    if (length(files.csv) != 0){
      databaseFileName <- file.path(dataFolder, "database.csv")
      validate(
        need("database.csv" %in% files.csv, "Cannot find file database.csv in zip file")
      )
      ...
      processFile <- function(file) {
        ...
        table <- readr::read_csv(file.path(dataFolder, file), col_types = readr::cols())
  ...
  output$selectdata <- renderUI({
    ...
    tagList(
      selectInput("target", "Target", unique(subset(exposureOfInterest, exposureId %in% datainfo()$loaddata$cohortMethodResult$targetId)$exposureName)),
      selectInput("comparator", "Comparator", unique(subset(exposureOfInterest, exposureId %in% datainfo()$loaddata$cohortMethodResult$comparatorId)$exposureName)),
  ...
  output$downloadKaplanMeierPlotPPTx <- downloadHandler(filename = "KaplanMeier.pptx", 
                                                       content = function(file) {
                                                         my_vec_graph <- rvg::dml(ggobj = ggpubr::as_ggplot(kaplanMeierPlot()))
                                                         doc <- officer::read_pptx()
                                                         doc <- officer::add_slide(doc, layout = "Title and Content", master = "Office Theme")
                                                         doc <- officer::ph_with(doc, my_vec_graph, location = officer::ph_location(width = input$width_km, height = input$height_km))
